...
$(function () {
  console.log("Its working"); // const url = "https://api.spoonacular.com/recipes/search?query="
  // const apiKey = "&apiKey=502be8db956a4d2581735d3ceced3501&diet=vegan"

  $("button").click(async () => {
    console.log("button pushed");
    const newIngredient = $("#searchBar").val();
    console.log(newIngredient); // const recipes = await getRecipes(newIngredient);
  ...
  async function processRequest(ingredient) {
    ...
    console.log(recipes); // We need to use Promise.all() using map() with promises
    // see this post: https://futurestud.io/tutorials/node-js-how-to-run-an-asynchronous-function-in-array-map

    detailedRecipes = await Promise.all(recipes.map(async recipe => {
  ...
  async function getRecipes(query) {
    try {
      ...
      return response.data.results; // return recipes.map(async(recipe) => {
